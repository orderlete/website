...
export default function AdminCategories() {
  ...
  const resetForm = () => {
    setFormData({ name: '', image_url: '', section: 'confectionary', display_order: categories.length, is_featured: false, featured_priority: '0', discount_percent: '0' });
    setEditingId(null);
  };
  ...
  const startEdit = (cat: any) => {
    ...
    setFormData({
      ...
      featured_priority: cat.featured_priority?.toString() || '0',
      discount_percent: cat.discount_percent?.toString() || '0'
    });
           ...
           >
              <div className="flex items-center justify-between mb-2">
                 <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-black text-lg text-gray-900">{editingId ? 'Edit Category' : 'Add Category'}</h2>
                 </div>
                 {editingId && (
                   <button type="button" onClick={resetForm} className="p-1 text-gray-400 hover:text-red-500"><Trash2 size={20} /></button>
                 )}
              </div>
                 ...
                 {categories.map((cat) => (
                   ...
                   >
                      ...
                      <button 
                        ...
                        className="absolute top-2 left-2 p-2 bg-gray-50 text-gray-900 rounded-xl opacity-0 group-hover:opacity-100 transition-all hover:bg-gray-900 hover:text-white"
                      >
                         <Target size={14} />
                      </button>
                      {cat.discount_percent > 0 && (
                        <div className="absolute bottom-2 right-2 px-2 py-1 bg-primary text-white rounded-md text-[9px] font-black uppercase shadow-lg">
                          -{cat.discount_percent}% OFF
                        </div>
                      )}
                   </motion.div>
                 ))}
